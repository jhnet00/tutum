    ...
    if (messages.length === 0) {
        ...
        return (
            <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
                <div className="mb-6 flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-purple-500 to-fuchsia-500 text-white shadow-xl">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                        src="/images/tutum-character-ai-sm.png"
                        alt="Tutum AI avatar"
                        className="h-full w-full object-cover"
                        onError={(event) => {
                            event.currentTarget.style.display = 'none';
                            const fallback = event.currentTarget.parentElement?.querySelector('[data-avatar-fallback]') as HTMLElement | null;
                            if (fallback) fallback.style.display = 'flex';
                        }}
                    />
                    <span data-avatar-fallback className="hidden h-full w-full items-center justify-center">
